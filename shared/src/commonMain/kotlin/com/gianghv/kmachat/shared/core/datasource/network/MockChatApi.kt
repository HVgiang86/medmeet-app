...
class MockChatApi(httpClient: HttpClient) {
    ...
    private val conversationList = mutableListOf(
        Conversation(
            id = "1",
            name = "Conversation 1",
    ...
    suspend fun generateResponse(userMessage: String, conversationId: String? = null): BaseResponse<Message> {
    ...
    suspend fun getConversationList(): BaseResponse<List<Conversation>> {
        delay(1500) // Simulate network delay
        return BaseResponse(
            code = 200, message = "OK", data = conversationList
        )
    }
